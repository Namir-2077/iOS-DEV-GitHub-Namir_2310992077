...
//print("The value of foo is \(foo)")
print("That'll happen because the variable foo is only in scope within the loop")
...
print("This particular code compiled because x is a global variable and is therefore in scope throughout the program. Variable x is defined in the global scope and is modified within the loop by adding 1 to its value each time through the loop. foo, on the other hand, is defined within the loop and is therefore only in scope within that loop.")
...
print(greeting(greeting: nil, name: "Alice"))
print(greeting(greeting: "Hi there", name: "Bob"))
